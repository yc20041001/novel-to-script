...
- 使用 Redis Session + HttpOnly Cookie 管理本地登录态，避免前端存储 session id。
- 使用 MySQL 持久化用户、项目、章节、生成任务、剧本版本和生成结果。
...
flowchart LR
  ...
  Frontend -->|POST /api/auth/login| Backend["FastAPI 后端"]
  Backend -->|Session / 生成缓存| Redis["Redis / InMemory fallback"]
  Backend -->|业务数据持久化| MySQL["MySQL"]
...
| 层级 | 技术 | 职责 |
| --- | --- | --- |
| 表现层 | React + Vite + Tailwind CSS + shadcn/ui + motion + Monaco Editor | 登录、章节输入、状态提示、YAML 展示、复制下载 |
| 应用层 | FastAPI + Pydantic + PyYAML | 登录认证、请求校验、AI 编排、结构校验、YAML 转换 |
| Session/缓存层 | Redis + InMemory fallback | 存储登录 Session 和生成结果缓存，本地 Redis 不可用时保持可演示 |
| 数据持久化层 | MySQL + PyMySQL | 保存用户、项目、章节、任务、剧本版本和生成记录 |
...
- 调用 `/api/auth/login`、`/api/auth/me` 和 `/api/auth/logout` 管理登录状态。
...
| config.py | 读取 DeepSeek、Redis、MySQL、前端地址等配置 |
| session_store.py | Redis SessionStore 和内存回退实现 |
| services/user_repository.py | MySQL 用户表和密码校验 |
| services/generation_repository.py | MySQL 生成业务数据持久化 |
| services/generation_cache_service.py | Redis 生成缓存 |
...
- 登录态通过 HttpOnly Cookie 保存，前端不将 session id 写入 localStorage。
- Redis 不可用时自动回退到内存 SessionStore，适合本地演示；生产环境应确保 Redis 可用。

需要注意：当前版本已经具备演示级登录界面，但暂未对 `/api/generate` 等业务接口做强制服务端鉴权。若部署到公网，需要增加接口鉴权、限流、日志脱敏和请求大小限制。
