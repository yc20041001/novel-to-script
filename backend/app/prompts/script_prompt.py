from app.models import GenerateOptions, NovelChapter
...
def _build_options_block(options: GenerateOptions | None) -> str:
    if options is None:
        return ""
    parts: list[str] = []
    if options.genre:
        parts.append(f"剧本类型：{options.genre}")
    if options.style:
        parts.append(f"改编风格：{options.style}")
    if options.target_scene_count:
        parts.append(f"目标场景数量：{options.target_scene_count}")
    if options.language:
        parts.append(f"语言：{options.language}")
    if not parts:
        return ""
    return "\n".join(parts) + "\n\n"
